(function($) {
       ...
       $.fn.hasChanged = function() {
               ...
               if (!this.length || !this.is(':input')) return false;

               // only care about first one
               var ip = this[0],
                       $t = this.first(),
                       name = $t.attr('name'),
                       type = (ip.nodeName === 'SELECT' ? 'select' : $t.attr('type') || 'text').toLowerCase(),
                       changed = false;
               ...
               switch (type) {
                       ...
                       case 'select':
                               ...
                               if ($t[0].multiple !== true && !$t.data('_defaultSet')) {
                                       if ($t[0].selectedIndex === 0)
                                               $t.children('option:first')[0].defaultSelected = true;
